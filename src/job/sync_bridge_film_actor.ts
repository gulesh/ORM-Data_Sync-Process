...
export async function update_bridge_film_actor_table(): Promise<void> {
    ...
    await outgoingSourceDB.manager.transaction( async (transactionManager) => {
        for (const film_actor_entry of film_actor_entries) {
            await add_update_bridge_film_actor(film_actor_entry, transactionManager);
        }
        //update the entry in sync table
        const sync_row_updated = await update_table_record('film_actor', transactionManager);
        console.log("sync_row", sync_row_updated);
    })
}

export async  function add_update_bridge_film_actor(row: any, manager: any): Promise<void> {
    const bridge_film_actor_record = await manager.getRepository('bridge_film_actor').findOneBy({
        film_key: row.film_id,
        actor_key: row.actor_id
    });
    if(!bridge_film_actor_record){
        await bridge_film_actor_sync_with_sqlite(row, manager);
    }
}
